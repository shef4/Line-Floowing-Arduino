...
LeftS = a.analogRead(LEFT_SENSOR_PIN);
CenterS = a.analogRead(CENTER_SENSOR_PIN);
RightS = a.analogRead(RIGHT_SENSOR_PIN);
%RightC;
%CenterC;
%LeftC;
SensorVector = size(3,100);
if LeftS > 4
    Left = 'B';
elseif  LeftS < 0.5
    Left = 'W';
...
if Right == 'B' && center == 'B' && Left == 'B'
   disp('straight');
elseif Right == 'W' && center == 'B' && Left == 'B'
   disp('left');
elseif Right == 'B' && center == 'B' && Left == 'W'
    disp('Right');
end
%SensorVector(:,coloum) = [LeftS CenterS RightS];
coloum = coloum +1;
end                  
